/**
 * Utilidades para cálculo de periodicidad de suscripciones (Backend)
 * 
 ...
 * 
 * Portado desde client/src/utils/subscriptionPeriodicity.ts
 */
...
/**
 * Infiere la periodicidad desde un array de meses activos
 * Útil para determinar periodicidad desde hechos existentes
 */
export function inferPeriodicity(activeMonths: number[]): Periodicity {
  const count = activeMonths.length;
  
  if (count === 12) return 'monthly';
  if (count === 4) {
    // Verificar que sean trimestres (intervalos de 3 meses)
    const sorted = [...activeMonths].sort((a, b) => a - b);
    const intervals = [];
    for (let i = 1; i < sorted.length; i++) {
      intervals.push(sorted[i] - sorted[i-1]);
    }
    if (intervals.every(i => i === 3)) return 'quarterly';
  }
  if (count === 2) {
    // Verificar que sean semestres (intervalo de 6 meses)
    const sorted = [...activeMonths].sort((a, b) => a - b);
    if (sorted.length === 2 && sorted[1] - sorted[0] === 6) return 'semiannual';
  }
  if (count === 1) return 'annual';
  
  // Fallback: si no coincide con patrón conocido, asume mensual
  return 'monthly';
}
